...
public class BlockLantern extends Block {
	...
	public boolean canPlaceBlockAt(World world, int x, int y, int z) {
		return world.isBlockSolidOnSide(x - 1, y, z, EAST, true) || world.isBlockSolidOnSide(x + 1, y, z, WEST, true) || world.isBlockSolidOnSide(x, y, z - 1, SOUTH, true) || world.isBlockSolidOnSide(x, y, z + 1, NORTH, true) || world.isBlockSolidOnSide(x, y + 1, z, DOWN, true) || this.canPlaceLanternOn(world, x, y - 1, z);
	}
	...
	public int onBlockPlaced(World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata) {
		...
		if (metadata == 2) {
			if (side == 1 && this.canPlaceLanternOn(world, x, y - 1, z)) {
				if (world.rand.nextBoolean())
					setMetadata = 0;
				else
					setMetadata = 1;
	...
	public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
		...
		this.setBlockBounds(xOffset, yOffset, zOffset, xOffset + 0.5F, yOffset + 0.875F, zOffset + 0.5F);
	}
